...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            wpe = nn.Embedding(config.sequence_length, config.embedding_dim),
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.transformer.wpe(pos) # position embeddings of shape (T, embedding_dim)
        tok_emb = self.transformer.wte(idx) # token embeddings of shape (B, T, embedding_dim)
        x = tok_emb + pos_emb
    ...
    def from_pretrained(cls, model_type):
        ...
        # n_layer, n_head and embedding_dim are determined from model_type
        config_args = {
            'gpt2':         dict(n_layer=12, n_head=12, embedding_dim=768),  # 124M params
            'gpt2-medium':  dict(n_layer=24, n_head=16, embedding_dim=1024), # 350M params
            'gpt2-large':   dict(n_layer=36, n_head=20, embedding_dim=1280), # 774M params
            'gpt2-xl':      dict(n_layer=48, n_head=25, embedding_dim=1600), # 1558M params
        }[model_type]
